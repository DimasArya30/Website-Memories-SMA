...
// Pagination number helper dengan ellipsis
...
export default function GallerySection() {
  ...
  // Buka modal — index relatif terhadap seluruh data
  const openModal = (pageItemIndex: number) => {
    setModalIndex(startIndex + pageItemIndex);
    setModalOpen(true);
  };

  // Tutup modal — bersihkan semua state
  ...
  // Scroll ke atas gallery saat ganti halaman
  ...
  return (
    <section ref={ref} id="gallery" className="relative py-28 sm:py-36 overflow-hidden">
            ...
            {currentItems.map((item, i) => (
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-4 translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 z-[2]">
                  <p className="text-white text-xs sm:text-sm font-sans font-medium truncate">
                    {item.title}
                  </p>
                </div>
        ...
        {totalPages > 1 && (
          ...
          >
            ...
            <button
              ...
              className="w-10 h-10 rounded-xl glass flex items-center justify-center text-white/60 hover:text-white hover:border-primary/40 transition-all duration-300 disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:text-white/60 disabled:hover:border-transparent text-sm font-sans"
            >
              <ChevronLeft size={18} />
            </button>
            ...
            {getPageNumbers(currentPage, totalPages).map((page, i) =>
              typeof page === 'string' ? (
                <span key={`ellipsis-${i}`} className="w-10 h-10 flex items-center justify-center text-white/30 text-sm font-sans">
                  ...
                </span>
              ) : (
                <button
                  ...
                  className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-sans font-medium transition-all duration-300 ${
                    currentPage === page
      ...
      {/* ========== MODAL / LIGHTBOX ========== */}
      {/* Conditional rendering — TIDAK di DOM saat tertutup */}
      {modalOpen && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          ...
          onClick={closeModal}
        >
          {/* CLOSE BUTTON — selalu terlihat, 56px, glass, hover ungu, scale animasi */}
          <button
            onClick={(e) => {
              e.stopPropagation();
              closeModal();
            }}
            className="absolute top-5 right-5 z-[10001] w-14 h-14 rounded-2xl glass-strong flex items-center justify-center text-white/70 hover:text-white hover:bg-primary/30 hover:scale-110 transition-all duration-300 cursor-pointer"
            aria-label="Close"
          >
          ...
          <button
            ...
            className="absolute left-4 sm:left-8 top-1/2 -translate-y-1/2 z-[10001] w-12 h-12 rounded-full glass-strong flex items-center justify-center text-white/70 hover:text-white hover:bg-primary/20 transition-all duration-300 cursor-pointer"
            aria-label="Previous"
          >
            <ChevronLeft size={24} />
          </button>
          ...
          <button
            ...
            className="absolute right-4 sm:right-8 top-1/2 -translate-y-1/2 z-[10001] w-12 h-12 rounded-full glass-strong flex items-center justify-center text-white/70 hover:text-white hover:bg-primary/20 transition-all duration-300 cursor-pointer"
            aria-label="Next"
          >
